...
int main(int argc, char**argv ){
    ...
    outputfile->cd();   //RICORDARE IL DETTAGLIO _OLD
    h_tpt->Write((nomecanale+"_top_pt_old").c_str());
    h_Tpt->Write((nomecanale+"_Tprime_pt_old").c_str());
    h_mH->Write((nomecanale+"_mHiggs_old").c_str());
    h_cutFlow->Write((nomecanale+"_cutflow_old").c_str());

    h_Hpt_1->Write((nomecanale+"_Higgs_1_pt_old").c_str());
    h_Hpt_2->Write((nomecanale+"_Higgs_2_pt_old").c_str());
    h_Hpt_3->Write((nomecanale+"_Higgs_3_pt_old").c_str());

    h_mupt->Write((nomecanale+"_mu_pt_old").c_str());
    h_ept->Write((nomecanale+"_e_pt_old").c_str());
    h_jett_pt->Write((nomecanale+"_jetmax_pt_old").c_str());
    h_nmu->Write((nomecanale+"_n_mu_old").c_str());
    h_ne->Write((nomecanale+"_n_e_old").c_str());

    h_dr->Write((nomecanale+"_deltar_old").c_str());
    h_mHf->Write((nomecanale+"_massa_higgs_finale_old").c_str());
